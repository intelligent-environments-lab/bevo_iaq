...
def takeMeasurement():
  ...
  def bigReset():
    ...
    time.sleep(0.5)
    reset()
    time.sleep(0.5) # note: needed after reset
  ...
  ret = -1
  for i in range(3): # Number of tries before re-establishing connection
    ...
    if ret == True:
      ...
      break
    print('measurement start unsuccessful, next try')
    reset()
    time.sleep(0.5)

  if ret == -1:
    print('measurement start unsuccessful, restarting communication')
    bigReset()
  ...
  if ret != -1:
    ...
    print("no communication established, quitting")
